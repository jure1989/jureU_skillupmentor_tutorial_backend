...
export abstract class AbstractService {
  ...
  async findAll(relations = []): Promise<any[]> {
    try {
      return this.repository.find({ relations }) // relations - iskanje vlog...npr.: roles
    } catch (error) {
      Logging.error(error)
  ...
  async findBy(condition, relations = []): Promise<any> {
    try {
      return this.repository.findOne({ where: condition, relations }) // relations - iskanje vlog...npr.: roles
    } catch (error) {
      Logging.error(error)
  ...
  async findById(id: string, relations = []): Promise<any> {
    try {
      ...
      if (!element) {
        throw new BadRequestException(`cannot find element with an id: ${id}`)
      }
  ...
  async paginate(page = 1, relations = []): Promise<PaginatedResult> {
    const take = 10 // number of users to be displayed on one page
